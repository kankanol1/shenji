    ...
    <div class="layout">
        ...
        <el-dialog
            ...
            <el-form :model="addRoleForm"
                ...
                <el-form-item label="科室名称" prop="officename">
                    <el-input v-model="addRoleForm.officename"></el-input>
                </el-form-item>
                <el-form-item label="科室描述" prop="description">
                    <el-input
                            type="textarea"
                            :rows="2"
                            placeholder="请输入科室描述内容"
                ...
                <el-form-item label="行政区划" prop="regioncode">
                    <el-select
                            style="width:100%"
                            v-model="addRoleForm.regioncode"
                            clearable
                            placeholder="请选择行政区划">
                        <el-option
                                v-for="item in officeRegionList"
                                :key="item.id"
                                :label="item.name"
                                :value="item.id">
                ...
                <el-form-item label="授权行政区划" prop="rlist">
                    <el-select
                            style="width:100%"
                            v-model="rlist"
                            @change="changeAddRoleCode"
                            multiple
                            clearable
                            placeholder="请选择授权行政区划">
                        <el-option
        ...
        <el-dialog
            ...
            <el-form :model="selectRoleForm"
                     label-width="30%">
                <el-form-item label="科室名称" prop="officename">
                    <el-input v-model="selectRoleForm.officename"></el-input>
                </el-form-item>
                <el-form-item label="科室描述" prop="description">
                    <el-input
                            type="textarea"
                ...
                <el-form-item label="行政区划" prop="regioncode">
                    <el-select
                            style="width:100%"
                            v-model="selectRoleForm.regioncode"
                            clearable
                            placeholder="请选择行政区划">
                        <el-option
                                v-for="item in officeRegionList"
    ...
    export default {
        ...
        data() {
            return {
                ...
                officeRegionList:[],//行政区划数据
                roleCodeData:[], //授权行政区划数据
                roleVData: [],
                roleEData: [],
                addRoleEData: [],
                editRoleEData: [],
        ...
        methods: {
            ...
            queryOfficeRegionList(){
                ...
                that.$http.post("/api/regioncode/selectChildren", that.querystring.stringify({
                    ...
                    if ((response.data.status & 1) == 1) {
                        that.officeRegionList = response.data.data;
                    } else if ((response.data.status & 1) != 1) {
                        that.officeRegionList = [];
                        that.$message({
                            showClose: true,
                            message: '行政区划读取失败',
                            type: 'warning'
            ...
            queryRoleList() {
                ...
                if(this.roleName){
                    params.officename = this.roleName;
                }
                ...
                if(this.roleDetail>-1){
                    params.detail = this.roleDetail;
                }
                that.tableLoading = true;
                that.$http.post("/api/office/selectNode", that.querystring.stringify(params)
                ).then(function (response) {
            ...
            showEditRole(index) {
                ...
                for(var i=0;i<this.roleCodeData.length;i++){
                    rArr.push(this.roleCodeData[i].id);
                }
                this.rlist = this.valueToRoleArray(this.selectRoleForm.rlist,rArr);
            ...
            arrayToRoleValue(array){
                var result="";
                var space = ","
                for(var i=0;i<array.length;i++){
                    result = result+array[i] +space;
                }
                var sqvalue = result.substring(0,result.length-1);
                return sqvalue;
            },
            valueToRoleArray(value,array){
                var result = [];
                if(value){
                    result = value.split(",");
                }
            ...
            changeAddRoleCode(val){
                this.addRoleForm.rlist = this.arrayToRoleValue(val);
            },
            changeEditRoleCode(val){
                this.selectRoleForm.rlist = this.arrayToRoleValue(val);
            },
            changeEditVType(val) {
            ...
            insertRole(formName) {
                if(this.addRoleForm){
                    var that = this;
                    that.$http.post("/api/office/insertNode", that.querystring.stringify(that.addRoleForm)
                    ).then(function (response) {
                        if ((response.data.status & 1) == 1) {
                            setTimeout(function(){
                                that.queryRoleList();
            ...
            updateRole() {
                if(this.selectRoleForm){
                    var that = this;
                    that.$http.post("/api/office/updateNode", that.querystring.stringify(that.selectRoleForm)
                    ).then(function (response) {
                        if ((response.data.status & 1) == 1) {
                            setTimeout(function(){
                                that.queryRoleList();
            ...
            sendDeleteRoleRequest(ids) {
                var that = this;
                that.$http.post("/api/office/deleteNode", that.querystring.stringify({
                        ids:ids,
                    })
                ).then(function (response) {
                    if ((response.data.status & 1) == 1) {
